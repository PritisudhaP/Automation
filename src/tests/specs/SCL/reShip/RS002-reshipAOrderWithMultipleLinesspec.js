...
describe("reship of an original order that has multiple lines flow", function () {
    ...
    it("Create a sales order with multiple lines each with qty =2, create reship order,complete fulfilment -TC002", function () {
        /* create a sales order via call center with multiple lines having qty=2,release the order
        ...
        //update the line quantity as 2
        callCenter.increaseLineQty(2,2);
        ...
        //verify the price on line2
        salesOrderCreate.getLinePriceAmount(2).then(function (price) {
            var linePrice = price;
            expect(linePrice).toEqual("$0.00");
            console.log("The reship order price is " + linePrice);
        });
