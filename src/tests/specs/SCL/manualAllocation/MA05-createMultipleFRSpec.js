...
describe('Verify cancel button functionality of manual allocation window', function() {
    ...
    it('Try to cancel manual alloaction - TC004', function () {
        ...
        browser.sleep(2000);
        commons.searchWithCriteria('SKU', 'contains', browser.params.mustShipSku1);
        ...
        //verify that 2 shipmenet requests were created
        callCenter.getShipementRequestCount().then(function (count) {
            var shipmentReqCount = count;
            console.log("shipmenet request count of order is"+shipmentReqCount);
            expect(shipmentReqCount).toEqual('2');
        })
