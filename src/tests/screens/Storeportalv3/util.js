...
module.exports = { 
	...
	fluentWaitForPresence : function(locValue)
	{
		...
		browser.wait(EC.presenceOf(locValue),10000);//same as waitforelement() , but this one should be priority 
	},
	
	fluentWaitForClickable : function(locValue)
	{
		var EC = protractor.ExpectedConditions;		// Waits for the element to be clickable in the dom. when the page is not loaded fully 
	...
	},//explicity wait //drop-down example 
	...
	getSONo : function(filePath)
	{
		...
		.then(function(text) {
			  ...
			  fs.writeFile(filePath, JSON.stringify(file), function (err) {
			    if (err) return console.log(err);
			    console.log(JSON.stringify(file, null, 2));
			    console.log('writing to ' + filePath);//write to json file 
