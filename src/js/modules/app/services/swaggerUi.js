...
angular
    ...
    .controller('swaggerUiController', ["$scope", "$http", "$location", "$anchorScroll", "$timeout", "swaggerClient", "swaggerModules", "swaggerTranslator",'$filter', function($scope, $http, $location, $anchorScroll, $timeout, swaggerClient, swaggerModules, swaggerTranslator,$filter) {
        ...
        $scope.$watch('apiFilter', function(val){
            $scope.resources = $filter('filter')( $scope.originalResourcesAry, {'name':val});
        });
         ...
         */
        function swaggerLoaded(swaggerUrl, swaggerType, filterString) {
            ...
            swaggerModules
                .execute(swaggerModules.PARSE, $scope.parser, filterString, swaggerUrl, swaggerType, swagger, $scope.trustedSources, parseResult)
        ...
        function swaggerParsed(parseResult) {
            ...
            swaggerModules
                ...
                .then(function() {
                    ...
                    //store this to perform filtering the api
                    $scope.originalResourcesAry = parseResult.resources;
        ...
        function watchData() {
            $scope.$watch('input', function(data) {
                ...
                if (data) {
                    swagger = data;
                    swaggerLoaded(null, 'application/' + $scope.inputType, $scope.filterString);
                }
            });
        }
...
angular
    .module('swaggerUi')
    .service('swaggerClient', ["$q", "$http", "swaggerModules","$rootScope", function($q, $http, swaggerModules, $rootScope) {
...
angular
    ...
    .service('swaggerParser', ["$q", "$sce", "$location", "swaggerModel", "swaggerTranslator", function($q, $sce, $location, swaggerModel, swaggerTranslator) {
         ...
         * Module entry point
         */
        this.execute = function(parserType, filterString, url, contentType, data, isTrustedSources, parseResult) {
            var deferred = $q.defer();
            if (data.swagger === '2.0' && (parserType === 'json' || (parserType === 'auto' && contentType === 'application/json'))) {
                trustedSources = isTrustedSources;
                try {
                    parseSwagger2Json(data, filterString, url, deferred, parseResult);
                } catch (e) {
                    deferred.reject({
                        code: 500,
         ...
         */
        function parseSwagger2Json(swagger, filterString, url, deferred, parseResult) {
            var map = {},
                form = {},
                resources = [],
                infos = swagger.info,
...
angular.module('swaggerUi').run(['$templateCache', function($templateCache) {
    ...
    $templateCache.put('templates/swagger-ui.html',
        '<div class="swagger-ui" aria-live="polite" aria-relevant="additions removals">  <div style="margin-top: 5px;" ><input type="text" style="width:100%; height:50px" ng-model="apiFilter" placeholder="Type here to search the API list"> </div><ul class="list-unstyled endpoints"> <li ng-repeat="api in resources track by $index" class="endpoint" ng-class="{active:api.open}" ng-include="\'templates/endpoint.html\'"></li> </ul> <div class="api-version clearfix" ng-if="infos">  <a ng-if="validatorUrl!=\'false\'" target="_blank" href="{{validatorUrl}}/debug?url={{url}}"><img class="pull-right swagger-validator" ng-src="{{validatorUrl}}?url={{url}}"></a> </div> </div>');
    ...
    //<div class="api-name"> <h3 ng-bind="infos.title"></h3> </div> <div class="api-description" ng-bind-html="infos.description"></div> <div class="api-infos"> <div class="api-infos-contact" ng-if="infos.contact"> <div ng-if="infos.contact.name" class="api-infos-contact-name"><span swagger-translate="infoContactCreatedBy" swagger-translate-value="infos.contact"></span></div> <div ng-if="infos.contact.url" class="api-infos-contact-url"><span swagger-translate="infoContactUrl"></span> <a href="{{infos.contact.url}}" ng-bind="infos.contact.url"></a></div> <a ng-if="infos.contact.email" class="api-infos-contact-url" href="mailto:{{infos.contact.email}}?subject={{infos.title}}" swagger-translate="infoContactEmail"></a> </div> <div class="api-infos-license" ng-if="infos.license"> <span swagger-translate="infoLicense"></span><a href="{{infos.license.url}}" ng-bind="infos.license.name"></a> </div> </div>
}]);
